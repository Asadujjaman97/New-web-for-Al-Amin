...
// Make sure the requested PDF actually exists inside the pdfs/ directory
// (prevents path traversal / requesting arbitrary files on the server).
$pdfsDir = realpath(__DIR__ . "/pdfs");
$requestedPath = realpath(__DIR__ . "/" . $pdf);

if (
    $requestedPath === false ||
    $pdfsDir === false ||
    strpos($requestedPath, $pdfsDir . DIRECTORY_SEPARATOR) !== 0
) {
    echo json_encode(["status" => "error", "message" => "Requested data sheet not found"]);
    exit;
}
...
try {
    ...
    $otpMail->isHTML(true);
    $otpMail->Subject = "Your Data Sheet Verification Code";
    $otpMail->Body = "
        <h3>Hi $name,</h3>
        <p>Your verification code to access the requested data sheet is:</p>
    ...
    try {
        ...
        $leadMail->setFrom("info@al-amin.com.bd", "Website Data Sheet Request");
        $leadMail->addAddress("info@al-amin.com.bd");

        $leadMail->isHTML(true);
        $leadMail->Subject = "New Data Sheet Request";
        $leadMail->Body = "
            <h3>Data Sheet Request</h3>
            <p><strong>Name:</strong> $name</p>
            <p><strong>Email:</strong> $email</p>
            <p><strong>Phone:</strong> $phone</p>
            <p><strong>Company:</strong> $company</p>
            <p><strong>Message:</strong> $message</p>
            <p><strong>Requested Data Sheet:</strong> $pdf</p>
        ";
        $leadMail->send();
    } catch (Exception $e) {
        error_log("Data sheet lead notification failed: " . $leadMail->ErrorInfo);
    }
